...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$x->QueryFieldsTV = array(   
		...
		"IF(    CHAR_LENGTH(`invoices1`.`total`), CONCAT_WS('',   `invoices1`.`total`), '') /* Expected amount (Ush) */" => "expected_amount",
	...
	$x->QueryFieldsFilters = array(   
		...
		"IF(    CHAR_LENGTH(`invoices1`.`year`), CONCAT_WS('',   `invoices1`.`year`), '') /* Year */" => "Year",
		"IF(    CHAR_LENGTH(`invoices1`.`month`), CONCAT_WS('',   `invoices1`.`month`), '') /* Month */" => "Month",
		"IF(    CHAR_LENGTH(`invoices1`.`total`), CONCAT_WS('',   `invoices1`.`total`), '') /* Expected amount (Ush) */" => "Expected amount (Ksh)",
		"`payments`.`paid_amount`" => "Paid amount (Ush)",
		"`payments`.`balance`" => "Balance (Ush)",
		"`payments`.`date`" => "Date",
		"`payments`.`comments`" => "Comments"
	);
	...
	$x->ColCaption = array("Tenant", "House", "Year", "Month", "Expected amount (Ush)", "Paid amount (Ush)", "Balance (Ush)", "Date", "Comments");
